            ...
            <div className="col-span-9 md:col-span-10 flex flex-col justify-between">
                ...
                <div className="flex justify-between items-end mt-4">
                    <div className="flex items-center border border-white/10">
                        <button onClick={() => updateQuantity(item.id, item.size, -1, item.isBundle, item.color)} className="p-2 hover:text-brand-volt"><Minus size={14} /></button>
                        <span className="w-10 text-center font-mono text-sm">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.size, 1, item.isBundle, item.color)} className="p-2 hover:text-brand-volt"><Plus size={14} /></button>
                    </div>

                    <button
                        onClick={() => moveToWishlist(item.id, item.size, item.isBundle, item.color)}
